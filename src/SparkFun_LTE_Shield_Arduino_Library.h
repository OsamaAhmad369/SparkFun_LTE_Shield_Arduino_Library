...
public:
    //  Constructor
#ifdef ESP32
    LTE_Shield(uint8_t powerPin = LTE_SHIELD_POWER_PIN, uint8_t resetPin = LTE_SHIELD_RESET_PIN,uint8_t TXPIN = TxPin,uint8_t RXPIN = RxPin,bool EN = LOW_ACTIVE_ENABLE);
#else
    LTE_Shield(uint8_t powerPin = LTE_SHIELD_POWER_PIN, uint8_t resetPin = LTE_SHIELD_RESET_PIN,bool EN = LOW_ACTIVE_ENABLE);
#endif
