...
#ifdef ESP32
LTE_Shield::LTE_Shield(uint8_t powerPin, uint8_t resetPin,uint8_t TXPIN,uint8_t RXPIN, bool EN)
{
...
#else
LTE_Shield::LTE_Shield(uint8_t powerPin, uint8_t resetPin, bool EN)
{
#ifdef LTE_SHIELD_SOFTWARE_SERIAL_ENABLED
    _softSerial = NULL;
#endif
...
void LTE_Shield::hwReset(void)
{
    ...
    if(!(ENABLE_SIG))
    {
    Serial.println("LOW RESET ENABLE");
    digitalWrite(_resetPin, LOW);
    delay(LTE_RESET_PULSE_PERIOD);
    digitalWrite(_resetPin, HIGH);
    }
    else{
    Serial.println("HIGH RESET ENABLE");
    digitalWrite(_resetPin, HIGH);
    delay(LTE_RESET_PULSE_PERIOD);
    digitalWrite(_resetPin, LOW);
    }
...
void LTE_Shield::beginSerial(unsigned long baud)
{
    ...
    {
#ifdef ESP32
        _softSerial->begin(baud,SWSERIAL_8N1,_rxPin,_txPin,false);
#else
